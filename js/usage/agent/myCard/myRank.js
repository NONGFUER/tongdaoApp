...
var rankParm = JSON.parse(UrlDecode(getUrlQueryString("jsonKey"))),
	customerId = rankParm.customerId,
	userCode = rankParm.userCode,
	transToken = rankParm.transToken;
/*var customerId = '20';
	...
	$(".rank_tab").find("li").bind("click", function() { //产品介绍 详情说明切换
		...
		if(tabIndex == 0) {
			...
			if(message <= 50 && message >= 1) {
				...
				$(".rightImg").attr("src", "../../../image/agent/myRank/rightlight.png");
				if(message) {
					$('#message').html('恭喜您，您排行第' + message + '位');
				}
				$('#message').addClass('on');
			} else {
				$(".rankImg img").attr("src", "../../../image/agent/myRank/loser.png");
			...
			if(countYear || countYear == 0) {
				$("#count").html('您当年签单：' + countYear + '单');
			}
			if(premYear) {
				$("#prem").html('您当年保费：' + premYear + '元');
			}
			if(rankYear <= 50 && rankYear >= 1) {
				...
				$(".rightImg").attr("src", "../../../image/agent/myRank/rightlight.png");
				if(rankYear) {
					$('#message').html('恭喜您，您排行第' + rankYear + '位');
				}
				$('#message').addClass('on');
			} else {
				$(".rankImg img").attr("src", "../../../image/agent/myRank/loser.png");
