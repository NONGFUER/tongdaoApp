...
$(function(){
	...
	$(".h_back").unbind("tap").bind("tap",function(){
		if(flag =='2'){
			window.location.href ="verify.html";//返回"点击开始认证申请"页面
		}else if(flag =='1'){
			window.location.href = base.url+'person.html';//返回"点击继续认证"
		}
	})
...
$.InfocallBack = function(data){
	...
	if(data.statusCode == "000000"){
		...
		if(param.bankPrivinceName&&param.bankCityName){
			$("#bankquyu_input").val(param.bankPrivinceName+" "+param.bankCityName);//所属区域
		}else{
			$("#bankquyu_input").val("请选择").css("color", "#ccc");
		}
		if(param.bankPrivinceName&&param.bankPrivinceCode){
	...
	if(data.statusCode == "000000"){
		$("#bankname_input").unbind("tap").bind("tap",function(){
			...
			selectPicker.show(function(items) {
				...
				bankCode = selectResult.name = items[0].value;			
				$("#bankquyu_input").attr("bankPrivinceCode",'').attr("bankPrivinceName",'').attr("bankCityCode",'').attr("bankCityName",'').val("请选择").css("color","#ccc");
				$("#zhihang_input").attr("name","").val("请选择").css("color", "#ccc");
				getbankquyuList();
			})
		})
...
$.saveAgentInfo = function(){
	...
	reqData = {
			"head":{
				...
				"recommendAgentCode":$("#introducer_input").val(),//推荐人工号
...
$.checkInfo = function(){
	...
	var reg = /^\d{16}|\d{19}$/;
	if(!reg.test( $("#bankcardNo_input").val())){
		modelAlert("银行卡号格式有误,请重新输入！");
		return false;
	}
	return true;
}
