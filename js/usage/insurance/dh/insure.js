...
$(function(){
	...
	if(urlParm.bzPic){
		if( ccId == "22"){
			var picStr =  '<a style="position: absolute;top: 0; left: 0;right: 0; bottom: 0;z-index: 2;" onclick="toHosUrl()"></a><img src="'+urlParm.bzPic+'" />';
		}else{
...
function getXGInfo(cityCode){
	...
	$.reqAjaxsChannel(url,reqData,getXGInfoCallback);	
}
...
function sendInsureRequest(){
	...
	if( ccId != "1" && ccId != "2" && ccId != "3"){
		if( urlParm.sourcePage == 'qudaoMall' || entry == 'qudao' ){
			...
			$.reqAjaxsChannel(url,reqData,insureReponse);
		}else{
...
function payRequest(serialNo){
	...
	if( urlParm.sourcePage == 'qudaoMall' || entry == 'qudao' ){
		$.toAjaxsChannel(url,reqData,payReturnReponse);
	}else{
...
function getServiceTime(){
	...
	$.reqAjaxsFalse(url,reqData,function(data){
	    if(data.statusCode == "000000") {
	        ...
	        if( ccId != "14" && ccId != "1" && ccId != "2" && ccId != "3" && ccId != "22" && ccId != "23" ){ //旅意险生效日其可修改	ecard生效日期可以修改  
