...
$(function(){
	...
	$("#ziyou").unbind("tap").bind("tap",function(){
		...
		//交强险默认选中
		jqxFlagg = "1";
		$(".toubao").css("color","#1b6bb8");
		$(".toubao").css("border-color","#1b6bb8");
		// 添加主附险内容
	...
	$(".toubao").unbind("tap").bind("tap",function(){
		...
		if(color == "rgb(27, 107, 184)"){
			$(".toubao").css("color","#aeb2b7");
			$(".toubao").css("border-color","#aeb2b7");
		}
		//获取是否投保交强险标志
		var picColor = $(".toubao").css("color");
		if(picColor == "rgb(27, 107, 184)"){
			jqxFlagg = "1";
		}else{
			jqxFlagg = "0";
		}
...
function cheackRiskinfo() {
	...
	// busiSelectedFlag = false;// 校验是否有选择商业主险险种标志
...
$.submitCallBack = function(paramList) {
	if(paramList.statusCode == "888888"){
		...
		queryCheckCode="";//商业险投保 验证码图片的base64字符串
		 ...
		 var exception = exception_translate(exception_filter(paramList.statusMessage));
         var carmodelkey = exception_getcarmodelkey(exception);
		 if(carmodelkey != ""){//车型选择与车辆真实型号不符
			modelAlert(exception,"",function(){
				sessionStorage.setItem("brandKey", carmodelkey);//搜索关键字
				window.location.href="vehiclequery.html"+window.location.search;
			});
			
		}else{
			modelAlert(exception);
		}
	}
};
// 添加主附险内容
