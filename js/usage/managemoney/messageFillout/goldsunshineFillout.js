...
$("#huifang").unbind("tap").bind("tap", function() {
	...
	if(huiclass == 'div_btn') {
		...
		var isidNo = $('#idNo').val();
		var leixing = $('#zhiyeleibie').attr('data-where');
		...
		if(chengshi == '请选择') {
			...
			modelAlert("请选择职业类别！");
			dui = false;
		} else if(leixing == '12100309' || leixing == '12020205') {
			modelAlert("五类,六类职业类别无法投保！");
			dui = false;
		} else if(panage > 60 || panage < 18) {
			modelAlert("投保人年龄必须为18至60周岁");
			dui = false;
		}
		if(dui) {
			...
			var redata = {
				...
				"body": {
					...
					"holderCardValid": youxiaoqi, //证件有效期
					...
					"invitePhone": shareMobile, //分享人手机号
					"orderResource": inFlag, //来源渠道
					"buyType": buyType, //1直接购买，2分享购买
					"customerId": customerId, //用户id
					"holderOccupationCode": leixing, //职业类别默认5010101
					"age": getAge_premium($.getBirthDay($('#idNo').val())) + '',
				}
			};
			var url = base.url + 'ygJmysunLife/sunLifeSaveOrder.do';
...
$(".bank").unbind("tap").bind("tap", function() {
	...
	window.location.href = "../cardList/goldsuncardList.html?jsonKey=" + jsonStr;
})
//获取联动列表
function getProvinceReq() {
	...
	var url = base.url + 'ygJmyBasic/getYgSelection.do';
	$.reqAjaxs(url, reqData, getSelection);
}
/*获取职业信息接口*/
function getzhiyeleixing() {
	$("#zhiyeleibie").unbind("tap").bind("tap", function() {
		vm.holderbr.name = $('#name').val();
		vm.holderbr.idNo = $('#idNo').val();
		vm.holderbr.phone = $('#phone').val();
		vm.holderbr.address = $('#dizhi').val();
		...
		urlParm.youxiaoqi = $('#shenfenzhengyxq').html();
		urlParm.pieces = $('#goumai').val();
		urlParm.title = "职业风险评估";
		urlParm.titles = vm.comComName;
		var jsonStr = UrlEncode(JSON.stringify(urlParm));
		window.location.href = base.url + "tongdaoApp/html/managemoney/messageFillout/goldsunzhiye.html?jsonKey=" + jsonStr;
	})
	...
	$("#zhiyeleibie").bind("tap", function() {
		cityPicker1.show(function(item) {
			...
			if($("#zhiyeleibie").html() == '五类' || $("#zhiyeleibie").html() == '六类') {
				modelAlert('五类，六类，职业类别无法投保');
			}
		});
	});*/
}
...
$("#commonHoldersto").unbind("tap").bind("tap", function() {
	...
	urlParm.titles = vm.comComName;
	urlParm.rightIco = "4";
	urlParm.frompage = "goldsunshineFilloutHtml";
	var jsonStr = UrlEncode(JSON.stringify(urlParm));
